/**
 * /admin/modules/posts/cron — schedule QStash del modulo Posts.
 */
import {
  QStashScheduleTable,
  type QStashRow,
...
const MODULE_PATH_PREFIX = "/api/cron/modules/posts/";
...
export default async function PostsCronPage() {
  ...
  const moduleJobnames = new Set(POSTS_MODULE.cronJobs.map((j) => j.jobname));

  const rows: QStashRow[] = CRON_SCHEDULES.filter(
    (def) => def.path.startsWith(MODULE_PATH_PREFIX) || moduleJobnames.has(def.jobname),
  ).map((def) => {
    const manifest = POSTS_MODULE.cronJobs.find((j) => j.jobname === def.jobname);
  ...
  return (
    <div className="space-y-5">
      ...
      <QStashScheduleTable rows={rows} emptyMessage="No Posts cron jobs configured." />
    </div>
  );
}
