/**
 * /admin/modules/news/cron — vista MODULO dei cron job pg_cron del modulo News.
 *
 * Stesso pattern di /admin/modules/posts/cron: metadati dal manifest,
 * "Missing jobs" warning con SQL pronto se non sono ancora installati su
 * pg_cron, table con last run + toggle enable/disable.
 */
import {
  CronJobsTable,
  type CronRow,
...
import { fetchNewsCronRunsAction, toggleNewsCronJobAction } from "./actions";
...
export default async function NewsCronPage() {
  ...
  const owned = getModuleJobnames("news");
  ...
  const missing = NEWS_MODULE.cronJobs
  ...
  return (
    <div className="space-y-5">
      ...
      <CronJobsTable
        rows={rows}
        toggleAction={toggleNewsCronJobAction}
        fetchRunsAction={fetchNewsCronRunsAction}
        emptyMessage="No News cron jobs are currently registered in pg_cron."
      />
    </div>
  );
}
