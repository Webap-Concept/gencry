...
// /admin/security/blocked-usernames. Spiega cosa fa il blocco, la
// pattern syntax, dove viene applicato, e quali dipendenze richiede
// (Postgres + cache in-memory 5min, niente Redis esterno).

import { Asterisk, Database, ShieldCheck, UserX } from "lucide-react";
...
export async function BlockedUsernamesGuide() {
  const t = await getTranslations("admin.security.blockedUsernames.guide");
  return (
    <div>
      <p style={{ margin: 0 }}>{t.rich("intro", richTags)}</p>

      <section style={sectionStyle}>
        <H icon={UserX}>{t("howHeading")}</H>
        <p style={{ margin: 0 }}>{t.rich("howBody", richTags)}</p>
      </section>

      <section style={sectionStyle}>
        <H icon={Asterisk}>{t("patternsHeading")}</H>
        <p style={{ margin: 0 }}>{t.rich("patternsBody", richTags)}</p>
      </section>
